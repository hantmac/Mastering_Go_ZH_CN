...
现在让我们来说说 `sync.RWMutex` 是如果改进 `sync.Mutex` 的。尽管使用 `sync.RWMutex` 互斥锁只允许有一个函数执行写操作，但您能有多个属于 `sync.RWMutex` 互斥锁的读取者。然而，有一件事您要注意：除非`sync.RWMutex` 互斥锁的所有读取者解锁，您不能为写操作给它上锁，这也是为了能允许多个读取者而不得不付出的最小代价。
...
`show()` 函数使用 `RLock()` 和 `RUnlock()` 函数是因为它的关键部分是用来读取共享变量的。因此，尽管需要 goroutines 能读取共享变量，但没一个能在不使用 `Lock()` 和 `Unlock()` 函数的情况下修改它。然而，`Lock()` 函数会被一直阻塞，只要有一个使用互斥体来读取共享变量。
